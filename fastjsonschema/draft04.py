...
class CodeGeneratorDraft04(CodeGenerator):
    ...
    def generate_format(self):
        ...
        is_regex_format = format_ in self.FORMAT_REGEXS
        is_regex = format_ == 'regex'
        if is_regex_format or is_regex:
            self.l('#format')
            with self.l('if isinstance({variable}, str):'):
                if is_regex_format:
    ...
    def generate_required(self):
        ...
        with self.l('if not all(prop in {variable} for prop in {required}):'):
            self.throw('{name} must contain {required} properties')
        ...
        for key, prop_definition in self._definition['properties'].items():
            with self.in_context(key):
                ...
                if isinstance(prop_definition, dict) and 'default' in prop_definition:
                    with self.l('else:'):
                        self.l('{variable}["{}"] = {}', key, repr(prop_definition['default']))
    ...
    def generate_dependencies(self):
        ...
        for key, values in self._definition["dependencies"].items():
            ...
            with self.l('if "{}" in {variable}_keys:', key):
                if values is False:
                    self.throw('{} in {name} must not be there', key)
                elif isinstance(values, list):
                    for value in values:
                        with self.l('if "{}" not in {variable}_keys:', value):
                            self.throw('{name} missing dependency {} for {}', value, key)
                else:
