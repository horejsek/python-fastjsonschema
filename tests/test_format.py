...
exc = JsonSchemaException('data must be date-time', value='{data}', name='data', definition='{definition}')
...
exc = JsonSchemaException('data must be hostname', value='{data}', name='data', definition='{definition}')
@pytest.mark.parametrize('value, expected', [
    ('', exc),
    ('LDhsjf878&d', exc),
    ('bla.bla-', exc),
...
exc = JsonSchemaException('data must be custom-format', value='{data}', name='data', definition='{definition}')
